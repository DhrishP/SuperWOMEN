        ...
        <div className="flex justify-center pt-40">
            <div className="max-w-sm w-full shadow-lg bg-white p-8 rounded-xl opacity-70">
                ...
                <div className="relative mt-10">
                    ...
                    <input type="email" id="newTodo" value={newTodo} onChange={(e) => setNewTodo(e.target.value)} onKeyUp={(e) => handleKeyUp(e.key)} className="block w-full pl-10 p-2 border-4 rounded-full bg-gray-600 text-white" placeholder="Enter Your Email" />
                </div>
                                            ...
                                            {(draggableProvided) => (
                                                ...
                                                >
                                                    <li key={item.id} className="w-full border-2 rounded-xl mt-2 hover:border-blue-300">
                                                        <input  type="checkbox" className="float-left block w-6 h-6 m-3" />
                                                        <button  onClick={() => handleDelete(index)} className="float-right w-7 h-7 m-2.5 rounded-2xl bg-red-700 text-gray-200 shadow-md hover:bg-red-500 hover:scale-105">x</button>
                                                        <label  className="block w-full p-3">{item.content}</label>
                                                    </li>
                                                </div>
                                            )}
                                        </Draggable>
